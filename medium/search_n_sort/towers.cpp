...
void solve()
{
    ...
    vl towers;
    // greedy O(n lgn) solution
    // place box on smallest possible top box on which u can place it
    for (ll i = 0; i < n; i++)
    {
        if (towers.empty() || v[i] >= towers.back())
            towers.emplace_back(v[i]);
        else
        {
            ll ind = upper_bound(all(towers), v[i]) - towers.begin();
            towers[ind] = v[i];
        }
    }
    cout << towers.size() << "\n";
