...
void build_seg(vl &a, ll v, ll tl, ll tr)
{
    ...
    else
    {
        ...
        segtree[v] = min(segtree[v * 2], segtree[v * 2 + 1]); // change function here
    }
}
...
ll queryseg(ll v, ll tl, ll tr, ll l, ll r)
{
    ...
    // Start : v=1,tl=0,tr=n-1
    if (l > r)
        return 2e9; // change identity here
    if (l == tl && r == tr)
        return segtree[v];
    ll tm = (tl + tr) / 2;
    return min(queryseg(v * 2, tl, tm, l, min(r, tm)), queryseg(v * 2 + 1, tm + 1, tr, max(l, tm + 1), r)); // change function here
}
